...
export default function CreateCourse() {
  ...
  const [basicInfo, setBasicInfo] = useState({
    ...
    rating: "",
    duration: "",
    fee: "", // Changed default to empty string for cleaner math
  ...
  const [curriculum, setCurriculum] = useState([
    {
      title: "",
      details: "",
      sections: [{ title: "", concepts: "", labs: "", tools: "" }],
    },
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...basicInfo,
        // Ensure fee is saved as a clean number or string
        fee: basicInfo.fee.toString().replace(/[^0-9]/g, ""),
        discount: basicInfo.discount, // Include discount in payload
