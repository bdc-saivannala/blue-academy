...
export default function PricingSection({ course }) {
  ...
  // 1. Original Fee (Database Value)
  const originalFee = parseInt(course.fee.toString().replace(/,/g, "")) || 0;

  // 2. Discount Percentage (Database Value or 0)
  const discountPercent = parseInt(course.discount || "0");

  // 3. Total Payable after Discount (The "Real" Price)
  const totalPayable = Math.round(originalFee * (1 - discountPercent / 100));

  // 4. Application Fee (5% of the Total Payable)
  const applicationFee = Math.round(totalPayable * 0.05);

  // 5. Program Fee (Remaining Amount)
  const programFee = totalPayable - applicationFee;
  ...
  const handlePayment = async (amount, description, enrollmentId) => {
    ...
    const options = {
      key: "rzp_test_j4ZAeykcjIt2c9", // Replace with env variable in production
      amount: amount * 100,
      currency: "INR",
      name: "Blue Academy",
      description: description,
            ...
            <div className="grid max-w-6xl gap-8 mx-auto md:grid-cols-3">
              ...
              <div className="relative p-8 transition-shadow bg-white border-2 border-slate-200 rounded-2xl hover:shadow-lg">
                ...
                <div className="mt-6">
                  <button
                    onClick={() =>
                      handleInitiate(applicationFee, "Application Fee")
                    }
                    className="w-full px-4 py-3 font-medium transition-colors rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200"
                  >
                    Start Application
              ...
              <div className="relative p-8 text-white transform shadow-xl bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl md:scale-105">
                ...
                <div className="mb-6">
                  ...
                  <p className="mt-1 mb-4 text-xs text-white/80">
                    (After Application Fee + {discountPercent}% Discount)
                  </p>
                  <div className="flex items-baseline gap-1">
                    {/* Show Remaining Program Fee */}
                    <span className="text-4xl font-bold">
                ...
                <button
                  onClick={() => handleInitiate(programFee, "Program Fee")}
                  className="w-full px-4 py-3 font-bold text-blue-700 transition-colors bg-white rounded-lg shadow-lg hover:bg-blue-50"
                >
                  Enroll Now
                </button>
              ...
              <div className="relative p-8 transition-shadow bg-white border-2 border-slate-200 rounded-2xl hover:shadow-lg">
                ...
                {(() => {
                  // Calculate EMI on the Program Fee + GST
                  const gstAmount = programFee * 0.18;
                  const amountForEMI = programFee + gstAmount;
                  const emi12 =
                    Math.round(
                      (amountForEMI *
                        (0.15 / 12) *
                ...
                <button
                  onClick={() => handleInitiate(500, "EMI Down Payment")}
                  className="flex items-center justify-center w-full gap-2 px-4 py-3 font-medium text-green-700 transition-colors bg-green-100 rounded-lg hover:bg-green-200"
                >
                  <Calculator className="w-4 h-4" /> Get Personalized EMI Plan
                </button>
